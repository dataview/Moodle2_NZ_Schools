    ...
    function cron_alt() {
        ...
        if (!empty($started)) {
            ...
            if ($started > $timetocheck) {
                mtrace('Gdata cron halted: cron is either still running or has not yet expired.  The cron will expire at '.userdate($started + $expire));
        ...
        try {
            ...
            mtrace('Synchronization halted: '.$e->getMessage());
