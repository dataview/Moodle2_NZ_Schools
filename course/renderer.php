...
class core_course_renderer extends plugin_renderer_base {
    ...
    protected function course_category_tree_category(stdClass $category, $depth=1) {
        ...
        $content .= html_writer::link(new moodle_url('/course/category.php', array('id'=>$category->id)), local_courseicon_icon_tag($category,'coursecategory','small').$category->name, array('class'=>'category_link'));
        $content .= html_writer::end_tag('div');
        if ($hassubcategories) {
